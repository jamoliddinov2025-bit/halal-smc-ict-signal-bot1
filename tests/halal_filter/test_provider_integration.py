...
PATH = Path(__file__).resolve().parents[2] / "config" / "halal-filter.example.toml"
...
def test_documented_csv_default_pipeline_and_stream_match():
    ...
    config = load_halal_filter_config(PATH)
    ...
    expected = analyze_halal(analyze_mtf(primary, frames, mtf), config)
    engine = HalalFilterAnalyzer(config)
    actual = tuple(engine.update(frame) for frame in analyze_mtf(primary, frames, mtf))
    assert actual == expected
    assert actual[0].classification is AssetClassification.HALAL
    assert actual[0].eligible is True
    assert actual[0].decision.symbol == "BTCUSDT"
...
def test_mocked_public_binance_normalization_uses_existing_providers():
    ...
    frames = analyze_halal(analyze_mtf(primary, higher()))
    assert frames[0].classification is AssetClassification.HALAL
    assert frames[0].eligible is True


def test_lowercase_series_symbol_normalizes_without_rewriting_series_identity():
    primary = _ote(
    ...
    frames = analyze_halal(analyze_mtf(primary, {"1h": hourly, "4h": four}, load_mtf_config(PATH)))
    assert frames[0].decision.symbol == "BTCUSDT"
    assert frames[0].classification is AssetClassification.HALAL
