...
def test_default_command_reports_analysis_scope(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert captured.out == (
        ...
        "Phase 6: deterministic Fair Value Gap analysis.\n"
...
def test_help_option(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "Phase 6 Fair Value Gap analysis" in output
    assert "Phase 7 requires explicit approval" in output
...
def test_module_entrypoint_works_outside_checkout(tmp_path: Path) -> None:
    ...
    assert "Phase 6: deterministic Fair Value Gap analysis." in result.stdout
