...
def test_default_command_reports_analysis_scope(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert captured.out == (
        ...
        "Phase 15: deterministic setup quality score.\n"
...
def test_help_option(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "Phase 15 deterministic" in output
    assert "Phase 16 requires explicit approval" in output
...
def test_module_entrypoint_works_outside_checkout(tmp_path: Path) -> None:
    ...
    assert "Phase 15: deterministic setup quality score." in result.stdout
