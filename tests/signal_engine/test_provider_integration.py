...
from smcsignal.analysis.setup_quality import analyze_setup_quality, load_setup_quality_config
...
PATH = Path(__file__).resolve().parents[2] / "config" / "signal-engine.example.toml"
...
def test_documented_csv_default_pipeline_and_stream_match():
    ...
    config = load_signal_engine_config(PATH)
    ...
    scored = analyze_setup_quality(analyze_halal(analyze_mtf(primary, frames, mtf), halal), quality)
    eligible = analyze_signal_eligibility(scored, eligibility)
    expected = analyze_signal_engine(eligible, config)
    engine = SignalEngineAnalyzer(config)
    actual = tuple(engine.update(frame) for frame in eligible)
    assert actual == expected
    assert actual[0].status is SignalStatus.NO_SIGNAL
    assert actual[4].status is SignalStatus.NO_SIGNAL
    assert actual[0].upstream is eligible[0]
...
def test_mocked_public_binance_normalization_uses_existing_providers():
    ...
    eligible = analyze_signal_eligibility(
        analyze_setup_quality(analyze_halal(analyze_mtf(primary, higher())))
    )
    frames = analyze_signal_engine(eligible)
    assert frames[0].status is SignalStatus.NO_SIGNAL
    assert frames[4].status is SignalStatus.NO_SIGNAL
    assert frames[4].candidate.signal.bias.value == "LONG_BIAS"


def test_lowercase_series_symbol_does_not_rewrite_identity_or_status():
    primary = _ote(
    ...
    scored = analyze_setup_quality(
        analyze_halal(analyze_mtf(primary, {"1h": hourly, "4h": four}, load_mtf_config(PATH)))
    )
    eligible = analyze_signal_eligibility(scored, load_signal_eligibility_config(PATH))
    frames = analyze_signal_engine(eligible, load_signal_engine_config(PATH))
    assert frames[0].upstream.upstream.upstream.decision.symbol == "BTCUSDT"
    assert frames[0].status is SignalStatus.NO_SIGNAL
