from copy import deepcopy
from dataclasses import replace
from decimal import Decimal
from random import Random

import pytest

from smcsignal.analysis.displacement import DisplacementConfig
from smcsignal.analysis.mitigation_blocks import analyze_mitigation_blocks
from tests.analysis.helpers import bar
from tests.breaker_blocks.helpers import multiple
from tests.mitigation_blocks.helpers import (
    DISPLACEMENT_ONLY,
    SERIES,
    analyzer,
    before_breaker,
    breaker_frames,
    bullish_mitigation,
    events,
    golden,
    run,
)


def seeded(seed, count=23):
    random = Random(seed)
    last = Decimal(100)
    result = []
    for index in range(count):
        close = max(Decimal(20), last + random.randint(-10, 10))
        result.append(
            bar(
                index,
                close,
                opening=last,
                high=max(last, close) + random.randint(0, 3),
                low=min(last, close) - random.randint(0, 3),
                volume=random.randint(0, 10),
            )
        )
        last = close
    return tuple(result)


@pytest.mark.parametrize("seed", [2, 17, 31])
@pytest.mark.parametrize("period", [1, 3, 14])
def test_every_prefix_equals_full_history_including_ids_and_hashes(seed, period):
    candles = seeded(seed)
    full = run(
        candles, displacement=DisplacementConfig(atr_period=period), order_blocks=DISPLACEMENT_ONLY
    )
    for cut in range(len(candles) + 1):
        prefix = run(
            candles[:cut],
            displacement=DisplacementConfig(atr_period=period),
            order_blocks=DISPLACEMENT_ONLY,
        )
        assert prefix == full[:cut]
        assert [e.event_id for e in events(prefix)] == [
            e.event_id for e in events(full) if e.confirmation_index < cut
        ]
        assert [f.provenance.input_prefix_hash for f in prefix] == [
            f.provenance.input_prefix_hash for f in full[:cut]
        ]


@pytest.mark.parametrize("seed", [5, 19])
@pytest.mark.parametrize("period", [3, 14])
def test_every_future_suffix_can_change_without_repainting_mitigation(seed, period):
    candles = seeded(seed)
    full = run(
        candles, displacement=DisplacementConfig(atr_period=period), order_blocks=DISPLACEMENT_ONLY
    )
    for cut in range(len(candles) + 1):
        future = tuple(bar(i, 1000 if i % 2 else 10) for i in range(cut, len(candles)))
        assert (
            run(
                (*candles[:cut], *future),
                displacement=DisplacementConfig(atr_period=period),
                order_blocks=DISPLACEMENT_ONLY,
            )[:cut]
            == full[:cut]
        )


def test_nonempty_default_both_direction_example_proves_prefix_stability():
    candles = golden()
    full = run(candles, displacement=DisplacementConfig())
    assert [(e.original_ob_confirmation_index, e.confirmation_index) for e in events(full)] == [
        (20, 21),
        (23, 24),
    ]
    for cut in range(len(candles) + 1):
        assert run(candles[:cut], displacement=DisplacementConfig()) == full[:cut]


def test_identical_obs_keep_original_ids_across_prefixes():
    candles = multiple()
    full = run(candles, displacement=DisplacementConfig(), order_blocks=DISPLACEMENT_ONLY)
    assert len(frames := [e for e in events(full) if e.confirmation_index == 18]) == 2
    assert frames[0].original_ob_id != frames[1].original_ob_id
    for cut in range(len(candles) + 1):
        assert (
            run(candles[:cut], displacement=DisplacementConfig(), order_blocks=DISPLACEMENT_ONLY)
            == full[:cut]
        )


def test_origins_are_not_mitigations_at_candidate_or_ob_publication_time():
    frames = run(bullish_mitigation())
    assert not frames[4].events and not frames[6].events
    event = frames[7].events[0]
    assert event.original_candidate_index == 4 and event.original_ob_confirmation_index == 6
    assert event.confirmation_index == 7
    assert event.original_ob_available_at <= event.interaction_timestamp
    assert event.interaction_closed_at <= event.available_at


def test_retained_mitigation_and_original_ob_never_mutate_after_future_interactions():
    candles = (*bullish_mitigation(), *(bar(i, 300 if i % 2 else 5) for i in range(8, 16)))
    source = breaker_frames(candles)
    engine = analyzer()
    original = tuple(engine.update(frame) for frame in source[:8])
    frozen = deepcopy(original)
    for frame in source[8:]:
        engine.update(frame)
    assert original == frozen == run(bullish_mitigation())


def test_future_breaker_does_not_change_already_published_mitigation():
    candles = before_breaker()
    full = run(candles)
    prefix = run(candles[:8])
    assert prefix == full[:8]
    assert events(prefix)[0].event_id == events(full)[0].event_id


def test_numerically_equal_decimal_spelling_preserves_all_ids():
    rewritten = tuple(
        replace(
            c,
            **{
                field: Decimal(str(getattr(c, field)) + ".00")
                for field in ("open", "high", "low", "close", "volume")
            },
        )
        for c in bullish_mitigation()
    )
    assert run(rewritten) == run(bullish_mitigation())


def test_changed_past_source_volume_changes_provenance_not_rule_definition():
    candles = bullish_mitigation()
    before = run(candles)
    after = run((replace(candles[0], volume=Decimal(9)), *candles[1:]))
    assert [
        (e.confirmation_index, e.direction, e.original_lower_boundary, e.original_upper_boundary)
        for e in events(before)
    ] == [
        (e.confirmation_index, e.direction, e.original_lower_boundary, e.original_upper_boundary)
        for e in events(after)
    ]
    assert {e.event_id for e in events(before)}.isdisjoint(e.event_id for e in events(after))


def test_source_namespace_binds_identity_without_rewriting_source_objects():
    source = breaker_frames(bullish_mitigation())
    initial = [f.provenance.evidence_id for f in source]
    first = analyze_mitigation_blocks(source)
    other = analyze_mitigation_blocks(
        breaker_frames(bullish_mitigation(), series=replace(SERIES, dataset_id="different-origin"))
    )
    assert initial == [f.provenance.evidence_id for f in source]
    assert {e.event_id for e in events(first)}.isdisjoint(e.event_id for e in events(other))
