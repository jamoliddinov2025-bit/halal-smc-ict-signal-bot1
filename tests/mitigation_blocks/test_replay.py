...
from smcsignal.analysis.mitigation_blocks import analyze_mitigation_blocks
...
def test_batch_stream_and_original_source_frames_are_identical():
    ...
    assert actual == analyze_mitigation_blocks(source)
...
def test_every_chunk_boundary_preserves_source_eligibility_and_interaction(cut):
    source = breaker_frames(bullish_mitigation())
    engine = analyzer()
    frames = []
    for chunk in (source[:cut], (), source[cut:]):
        frames.extend(engine.update(frame) for frame in chunk)
    assert tuple(frames) == analyze_mitigation_blocks(source)


def test_no_duplicate_upstream_detectors_are_run_by_mitigation_consumer():
    ...
    assert tuple(frames) == analyze_mitigation_blocks(breaker_frames(bullish_mitigation()))
    assert (
        ...
        == mitigation.processed_count
        == len(bullish_mitigation())
    )
...
def test_empty_and_single_pass_inputs():
    class Once:
        ...
        def __iter__(self):
            ...
            self.used = True
            yield from breaker_frames(bullish_mitigation())

    assert analyze_mitigation_blocks(Once()) == analyze_mitigation_blocks(
        breaker_frames(bullish_mitigation())
    )
    assert analyze_mitigation_blocks(()) == ()
    engine = analyzer()
...
def test_independent_interleaved_analyzers_do_not_share_interaction_state():
    left, right = analyzer(), analyzer()
    for frame in breaker_frames(bullish_mitigation()):
...
def test_invalid_input_does_not_consume_ob_or_advance_state(bad):
    source = breaker_frames(bullish_mitigation())
    engine = analyzer()
    before = tuple(engine.update(frame) for frame in source[:7])
    ...
    assert (*before, engine.update(source[7])) == analyze_mitigation_blocks(source[:8])
...
def test_conflicting_source_context_cannot_be_combined(change):
    ...
    original = breaker_frames(candles)
    kwargs = {
    ...
    other = breaker_frames(revised, **kwargs)
    engine = analyzer()
...
def test_visible_liquidity_configuration_cannot_change():
    source = breaker_frames(bullish_mitigation())
    changed = breaker_frames(bullish_mitigation(), liquidity=LiquidityConfig("USDT", Decimal(1)))
    engine = analyzer()
...
def test_time_gaps_do_not_invent_missing_ob_or_interaction_candles():
    candles = tuple(
        replace(c, timestamp=c.timestamp + timedelta(days=2 if i >= 7 else 0))
        for i, c in enumerate(bullish_mitigation())
    )
    actual = analyze_mitigation_blocks(breaker_frames(candles))
    assert [e.confirmation_index for e in events(actual)] == [7]
    assert actual[6].upstream.upstream.upstream.observation.reference.closed_at == candles[
        6
...
def test_invalid_raw_values_remain_rejected_by_approved_data_models():
    for fields in (
        ...
        with pytest.raises(DataValidationError):
            replace(bullish_mitigation()[0], **fields)


def test_noniterable_batch_fails():
    with pytest.raises(AnalysisInputError):
        analyze_mitigation_blocks(None)
