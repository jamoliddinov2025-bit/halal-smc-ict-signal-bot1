...
@pytest.mark.parametrize("cut", range(11))
def test_every_chunk_boundary_is_identical(cut):
    ...
    assert tuple(outputs) == analyze_ote(frames)


def test_full_pipeline_stream_is_not_recomputed_by_ote():
    ...
    ote = analyzer()
    result = tuple(
        ote.update(pd.update(blocks.update(gaps.update(displacement.update(liquidity.update(c))))))
        for c in golden()
    )
...
def test_empty_and_single_pass_inputs():
    ...
    assert analyze_ote(Once()) == run(golden())
    assert analyze_ote(()) == ()
    engine = analyzer()
...
def test_invalid_raw_inputs_are_atomic(value):
    frames = upstream(golden())
    engine = analyzer()
    before = tuple(engine.update(f) for f in frames[:5])
    ...
    assert (*before, engine.update(frames[5])) == analyze_ote(frames[:6])
...
def test_visible_liquidity_config_is_bound():
    ...
    for f in source[:4]:
        engine.update(f)
    with pytest.raises(AnalysisInputError):
        engine.update(changed[4])


def test_elapsed_gaps_do_not_pad_or_guess_ote_zones():
    candles = tuple(
