...
PATH = Path(__file__).resolve().parents[2] / "config" / "signal-eligibility.example.toml"
...
def test_documented_csv_default_pipeline_and_stream_match():
    ...
    config = load_signal_eligibility_config(PATH)
    ...
    expected = analyze_signal_eligibility(scored, config)
    engine = SignalEligibilityAnalyzer(config)
    actual = tuple(engine.update(frame) for frame in scored)
    assert actual == expected
    assert actual[0].status is EligibilityStatus.NOT_ELIGIBLE
    assert actual[0].bias is MarketBias.NEUTRAL
    assert actual[4].status is EligibilityStatus.NOT_ELIGIBLE
    assert actual[4].bias is MarketBias.LONG_BIAS
    assert actual[0].upstream is scored[0]
...
def test_mocked_public_binance_normalization_uses_existing_providers():
    ...
    assert frames[0].status is EligibilityStatus.NOT_ELIGIBLE
    assert frames[0].bias is MarketBias.NEUTRAL
    assert frames[4].bias is MarketBias.LONG_BIAS


def test_lowercase_series_symbol_does_not_rewrite_identity_or_eligibility():
    primary = _ote(
    ...
    frames = analyze_signal_eligibility(scored, load_signal_eligibility_config(PATH))
    assert frames[0].upstream.upstream.decision.symbol == "BTCUSDT"
    assert frames[0].bias is MarketBias.NEUTRAL
